...
//
// Persons: None
// Online Sources: I used the javadocs to help make this class
/////////////// (https://cs300-www.cs.wisc.edu/wp/wp-content/uploads/2023/fall/p09/BusStopTree.html)
//
///////////////////////////////////////////////////////////////////////////////
...
public class BusStopTree {
  ...
  public static boolean isValidBST(Node<Bus> node) {
    ...
    // get values
    Bus currentNode = node.getValue();
    Node<Bus> left = node.getLeft();
    Node<Bus> right = node.getRight();

    // compare subtrees
    if (left != null) {
      Bus maxLeft = findMaxValue(left);
      if (maxLeft.compareTo(currentNode) >= 0) {
        return false;
      }
    }
    ...
    // compare subtrees
    if (right != null) {
      Bus minRight = findMinValue(right);
      if (minRight.compareTo(currentNode) <= 0) {
        return false;
      }
    }

    // check trees with recursion
    return isValidBST(left) && isValidBST(right);
  }

  /**
   * getter method for finding max value
   * 
   * @param node - node from tree
   * @return max value
   */
  private static Bus findMaxValue(Node<Bus> node) {
  ...
  /**
   * getter method for finding min value
   * 
   * @param node - node from tree
   * @return min value
   */
  private static Bus findMinValue(Node<Bus> node) {
  ...
  public boolean addBusHelper(Node<Bus> node, Bus bus) {
    if (node == null) {
      // no node, create new bus
      node = new Node<>(bus);
      return true;
    }

    // compare node and bus
  ...
  public boolean containsHelper(Node<Bus> node, Bus bus) {
    ...
    if (compare == 0) {
      ...
      // less than node, search left subtree
  ...
  protected Node<Bus> getFirstNodeAfter(LocalTime time, Node<Bus> node) {
    ...
    // get time of bus
    LocalTime arriveTime = node.getValue().getArrivalTime();

    // compare time
    if (arriveTime.compareTo(time) >= 0) {
      // check for earlier time
      Node<Bus> leftTree = getFirstNodeAfter(time, node.getLeft());
  ...
  private int heightHelper(Node<Bus> node) {
    // check for height of empty subtree
    if (node == null) {
      return 0;
    }

    // get height of both subtrees
    int rTreeHeight = heightHelper(node.getRight());
    int lTreeHeight = heightHelper(node.getLeft());

    if (rTreeHeight > lTreeHeight) {
      // return height plus 1 to account for node
