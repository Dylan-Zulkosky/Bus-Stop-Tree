...
//////////////// (https://cs300-www.cs.wisc.edu/wp/wp-content/uploads/2023/fall/p09/Bus.html)
//
///////////////////////////////////////////////////////////////////////////////
...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    // get arrival time
    LocalTime firstArrive = this.getArrivalTime();
    LocalTime secondArrive = other.getArrivalTime();

    // compare times
    int arrivals = firstArrive.compareTo(secondArrive);
    if (arrivals != 0) {
      return arrivals; // return the comparison result of arrival times
    }

    // get route names
    String firstName = this.route.getRouteAndTripName();
    String secondName = other.route.getRouteAndTripName();

    // compare names
    int routes = firstName.compareTo(secondName);
    if (routes != 0) {
      return routes; // Return the comparison result of route names
    }

    // get direction
    int direction = this.route.getDirection().compareTo(other.route.getDirection());

    // compare direction
    if (direction != 0) {
      return direction; // Return the comparison result of directions
    }
